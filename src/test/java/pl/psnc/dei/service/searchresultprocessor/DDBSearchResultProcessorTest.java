...
public class DDBSearchResultProcessorTest {
	...
	@InjectMocks
	private DDBSearchResultProcessor ddbSearchResultProcessor = new DDBSearchResultProcessor(ddbRestService, ddbFormatResolver, recordDataCache);
	...
	private void setupRestResponse() {
		when(ddbRestService.retrieveRecordFromDDBAndConvertToJsonLd(anyString())).thenReturn(ddbRestResponse);
	}

	private void setupServiceUnavailable() {
		when(ddbRestService.retrieveRecordFromDDBAndConvertToJsonLd(anyString())).thenThrow(new RuntimeException());
	}
	...
	public void fillDataFromCache() {
		...
		verifyZeroInteractions(ddbRestService);
		Assert.assertEquals(searchResult, result);
		Assert.assertEquals("image/jpeg", result.getFormat());
		Assert.assertEquals("test", result.getTitle());
		Assert.assertEquals(DATA_UNAVAILABLE_VALUE, result.getAuthor());
