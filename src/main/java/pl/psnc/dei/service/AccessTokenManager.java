...
public class AccessTokenManager extends RestRequestExecutor {
	...
	public String getAccessTokenWithRefreshToken() {
		...
		String authResponse = webClient.post()
				...
				.retrieve()
				.onStatus(HttpStatus::is4xxClientError, clientResponse -> {
					logger.warn("Error {} while retrieving access token. Cause: {}", clientResponse.rawStatusCode(), clientResponse.statusCode().getReasonPhrase());
					return Mono.empty();
				})
				.onStatus(HttpStatus::is5xxServerError, clientResponse -> {
					logger.error("Error {} retrieving access token using refresh token. Cause: {}", clientResponse.rawStatusCode(), clientResponse.statusCode().getReasonPhrase());
					return Mono.error(new DEIHttpException(clientResponse.rawStatusCode(), clientResponse.statusCode().getReasonPhrase()));
				})
				.bodyToMono(String.class)
				.doOnError(throwable -> logger.error(throwable.getMessage()))
